...
release-build: go-get-gox
	...
	env CGO_ENABLED=0 gox -verbose \
		...
		-output="release/${VERSION}/{{.OS}}_{{.Arch}}/terraform-provider-logtail_v${VERSION}" \
		.
	( \
		cd release/${VERSION} && \
		for q in $$(ls -d */ | cut -d/ -f1); do (cd $$q; zip "../terraform-provider-logtail_${VERSION}_$$q.zip" *); done && \
		shasum -a 256 *.zip > terraform-provider-logtail_${VERSION}_SHA256SUMS \
	)
...
release-sign:
	...
	( \
		cd release/${VERSION} && \
		gpg --detach-sign terraform-provider-logtail_${VERSION}_SHA256SUMS \
	)
...
release: go-get-github-release clean gen release-build release-sign
	...
	github-release release --user betterstackhq --repo terraform-provider-logtail --tag "v${VERSION}" \
		--name "v${VERSION}" --description "[CHANGELOG](https://github.com/betterstackhq/terraform-provider-logtail/blob/master/CHANGELOG.md)" && \
	\
	github-release upload --user betterstackhq --repo terraform-provider-logtail --tag "v${VERSION}" \
		--name "terraform-provider-logtail_${VERSION}_SHA256SUMS" --file "release/${VERSION}/terraform-provider-logtail_${VERSION}_SHA256SUMS" && \
	github-release upload --user betterstackhq --repo terraform-provider-logtail --tag "v${VERSION}" \
		--name "terraform-provider-logtail_${VERSION}_SHA256SUMS.sig" --file "release/${VERSION}/terraform-provider-logtail_${VERSION}_SHA256SUMS.sig" && \
	\
	for qualifier in linux_amd64 linux_arm64 darwin_amd64 darwin_arm64 windows_amd64; do \
		github-release upload --user betterstackhq --repo terraform-provider-logtail --tag "v${VERSION}" \
			--name "terraform-provider-logtail_${VERSION}_$$qualifier.zip" --file "release/${VERSION}/terraform-provider-logtail_${VERSION}_$$qualifier.zip"; \
	done
