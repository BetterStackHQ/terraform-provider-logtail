...
`examples/` itself is the runnable **basic** example. `examples/connection/` runs as its own E2E config because it needs a global API token; the combined run denylists it, `logtail_source_aws_account` (needs AWS credentials), `logtail_source_gcp_project` (needs GCP credentials), and `logtail_source_azure_account` (needs a consented Azure tenant).
