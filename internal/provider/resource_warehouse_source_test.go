...
func TestResourceWarehouseSourceCustomBucket(t *testing.T) {
	var data atomic.Value
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		t.Log("Received " + r.Method + " " + r.RequestURI)
	...
	var name = "Test Warehouse Source with Custom Bucket"
		...
		Steps: []resource.TestStep{
			// Step 1 - create with custom_bucket
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttr("logtail_warehouse_source.this", "name", name),
					resource.TestCheckResourceAttr("logtail_warehouse_source.this", "token", "generated_by_logtail"),
					resource.TestCheckResourceAttr("logtail_warehouse_source.this", "custom_bucket.#", "1"),
					resource.TestCheckResourceAttr("logtail_warehouse_source.this", "custom_bucket.0.name", "my-test-bucket"),
					resource.TestCheckResourceAttr("logtail_warehouse_source.this", "custom_bucket.0.endpoint", "https://s3.amazonaws.com"),
					resource.TestCheckResourceAttr("logtail_warehouse_source.this", "custom_bucket.0.access_key_id", "AKIAIOSFODNN7EXAMPLE"),
					resource.TestCheckResourceAttr("logtail_warehouse_source.this", "custom_bucket.0.secret_access_key", "wJalrXUtnFEMI/K7MDENG/bPxRfiCYEXAMPLEKEY"),
					resource.TestCheckResourceAttr("logtail_warehouse_source.this", "custom_bucket.0.keep_data_after_retention", "false"),
				),
			},
			// Step 2 - update with custom_bucket and change other field (should preserve custom_bucket)
			{
				Config: fmt.Sprintf(`
				provider "logtail" {
					api_token = "foo"
...
func TestResourceWarehouseSourceCustomBucketKeepData(t *testing.T) {
	var data atomic.Value
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		t.Log("Received " + r.Method + " " + r.RequestURI)
	...
	var name = "Test Warehouse Source with Custom Bucket Keep Data"
		...
		Steps: []resource.TestStep{
			// Step 1 - create with custom_bucket and keep data after retention
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttr("logtail_warehouse_source.this", "custom_bucket.0.access_key_id", "AKIAIOSFODNN7EXAMPLE"),
					resource.TestCheckResourceAttr("logtail_warehouse_source.this", "custom_bucket.0.secret_access_key", "wJalrXUtnFEMI/K7MDENG/bPxRfiCYEXAMPLEKEY"),
					resource.TestCheckResourceAttr("logtail_warehouse_source.this", "custom_bucket.0.keep_data_after_retention", "true"),
				),
			},
		},
	})
...
func TestResourceWarehouseSourceCustomBucketRemovalValidation(t *testing.T) {
	var data atomic.Value
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		t.Log("Received " + r.Method + " " + r.RequestURI)
	...
	var name = "Test Warehouse Source Custom Bucket Removal"
