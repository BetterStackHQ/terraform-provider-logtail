...
func TestResourceSourceAWSAccount(t *testing.T) {
	...
	bodyContains := func(needle string) resource.TestCheckFunc {
		return func(s *terraform.State) error {
			body := string(lastRequestBody.Load().([]byte))
			if !strings.Contains(body, needle) {
				return fmt.Errorf("request body should contain %s, got: %s", needle, body)
			}
			return nil
		}
	}
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			// Step 1 - create an aws source and connect the account with new credentials.
			{
				Config: `
				provider "logtail" {
					api_token = "foo"
				...
				`,
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckResourceAttrSet("logtail_source.aws", "id"),
					resource.TestCheckResourceAttr("logtail_source.aws", "platform", "aws"),
					resource.TestCheckResourceAttr("logtail_source.aws", "aws_role_arn", "arn:aws:iam::123456789012:role/BetterStackIntegrationRole"),
					bodyContains(`"aws_role_arn":"arn:aws:iam::123456789012:role/BetterStackIntegrationRole"`),
					bodyContains(`"aws_external_id":"ext-123"`),
				),
			},
			// Step 2 - rotate the credentials; they must be sent on update.
			{
				Config: `
				provider "logtail" {
					api_token = "foo"
				...
				`,
				Check: resource.ComposeTestCheckFunc(
					bodyContains(`"aws_role_arn":"arn:aws:iam::123456789012:role/RotatedRole"`),
					bodyContains(`"aws_external_id":"ext-456"`),
				),
			},
		},
	})
...
func TestResourceSourceAWSAccountRejectedOnNonAWSPlatform(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		Steps: []resource.TestStep{
			{
				...
				resource "logtail_source" "not_aws" {
					name         = "Ubuntu"
					platform     = "ubuntu"
					aws_role_arn = "arn:aws:iam::123456789012:role/BetterStackIntegrationRole"
				}
