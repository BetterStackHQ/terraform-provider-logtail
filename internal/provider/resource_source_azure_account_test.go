...
func TestResourceSourceAzureAccount(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case r.Method == http.MethodPost && r.RequestURI == prefix:
			...
			// The Azure linkage param must NOT ride along on the source create - it only
			// reaches the API via the separate logtail_source_azure_account PATCH.
			if strings.Contains(string(body), "azure_account_id") {
				t.Fatalf("source create body should not carry the Azure linkage param, got: %s", string(body))
			}
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			// Step 1 - create the source and link the connected tenant by account ID.
			{
				Config: `
				provider "logtail" {
					api_token = "foo"
				...
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckResourceAttr("logtail_source_azure_account.azure", "id", "1"),
					resource.TestCheckResourceAttr("logtail_source_azure_account.azure", "source_id", "1"),
					resource.TestCheckResourceAttr("logtail_source_azure_account.azure", "azure_account_id", "42"),
					patchBodyContains(`"azure_account_id":"42"`),
				),
			},
			// Step 2 - relink a different tenant in place; the PATCH must carry the new ID.
			{
				Config: `
				provider "logtail" {
					api_token = "foo"
				...
				`,
				Check: resource.ComposeTestCheckFunc(
					patchBodyContains(`"azure_account_id":"43"`),
				),
			},
